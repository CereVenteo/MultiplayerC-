...
void Spaceship::onInput(const InputController &input)
{
	...
	if (input.actionLeft == ButtonState::Press)
	{
		if (isServer)
		{
			...
			laser->sprite->texture = App->modResources->bullet;

			Laser *laserBehaviour = App->modBehaviour->addLaser(laser);
			laserBehaviour->father = gameObject;
...
void Spaceship::update()
{
	...
	if (gameObject->dead)
	{
		gameObject->secondsToRespawn += Time.deltaTime;
	}

	if (gameObject->secondsToRespawn >= RESPAWN_SECONDS)
	{
		gameObject->dead = false;
		gameObject->secondsToRespawn = 0.0f;
...
void Spaceship::onCollisionTriggered(Collider &c1, Collider &c2)
{
	if (c2.type == ColliderType::Laser && c2.gameObject->tag != gameObject->tag)
	{
		if (isServer)
		{
			GameObject* helper = c2.gameObject->behaviour->father;
			...
			if (hitPoints <= 0 && !gameObject->dead)
			{
				...
				App->modNetServer->ScoreSearcher(helper);
				App->modNetServer->ScoreReset(gameObject);
			...
			explosion->sprite->texture = App->modResources->explosion;
			explosion->sprite->order = 100;


			// we should do a animation transfere xd
