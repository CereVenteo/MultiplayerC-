...
struct GameObject
{
	...
	//Serialization
	void WCreate(OutputMemoryStream& packet);
	void WUpdate(OutputMemoryStream& packet);
	void RCreate(const InputMemoryStream& packet);
	void RUpdate(const InputMemoryStream& packet);

	void Interpolation();
	vec2 init_pos = vec2{ 0.0f, 0.0f };
	float init_ang = 0.0f;

	vec2 final_pos = vec2{ 0.0f, 0.0f };
	float final_ang = 0.0f;

	float seconds_elapsed = 0.0f;

	float secondsToRespawn = 0.0f;
	bool dead = false;
