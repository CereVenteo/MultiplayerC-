...
void ModuleNetworkingServer::onPacketReceived(const InputMemoryStream &packet, const sockaddr_in &fromAddress)
{
	if (state == ServerState::Listening)
	{
		...
		else if (message == ClientMessage::Ping)
		{
			...
			if (proxy != nullptr)
			{
				OutputMemoryStream p_pong;
				p_pong << PROTOCOL_ID;
				p_pong << ServerMessage::Ping;
				proxy->delivery_manager.processAckSequenceNumbers(packet);
				sendPacket(p_pong, fromAddress);
	...
	if (state == ServerState::Listening)
	{
		...
		for (ClientProxy &clientProxy : clientProxies)
		{
			if (clientProxy.connected)
			{
				...
				// If the client proxy doesnt have a gameobjects means that the client died
				//if (!clientProxy.gameObject)
				//{
				//	clientProxy.secondsSinceIdied += Time.deltaTime;
				//}

				//// time to wake up samurai
				//if (clientProxy.secondsSinceIdied >= RESPAWN_SECONDS)
				//{
				//	clientProxy.secondsSinceIdied = 0.0f;
				//	vec2 initialPosition = 500.0f * vec2{ Random.next() - 0.5f, Random.next() - 0.5f };
				//	float initialAngle = 360.0f * Random.next();
				//	clientProxy.gameObject = spawnPlayer(clientProxy.spaceship_type, initialPosition, initialAngle);
				//	OutputMemoryStream packet;
				//	packet << PROTOCOL_ID;
				//	packet.Write(ServerMessage::Respawn);
				//	packet << clientProxy.gameObject->networkId;

				//}

				clientProxy.secondsSinceLastPacket += Time.deltaTime;
				clientProxy.secondsSinceLastUpdate += Time.deltaTime;
				...
				// Cada cierto tiempo enviamos un paquete a la peña para que sepa que cosas estan pasando por el mundo
				// TODO(you): World state replication lab session
				if (clientProxy.secondsSinceLastUpdate >= REPLICATION_SECONDS)
				{
					//Aqui enviamos la mierda.
					//Reseteamo el timer as always
					clientProxy.secondsSinceLastUpdate = 0.0f;
					OutputMemoryStream packet;
					packet << PROTOCOL_ID;
					packet.Write(ServerMessage::Replication);
					packet << clientProxy.marcador;
					packet << clientProxy.nextExpectedInputSequenceNumber - 1;

					Delivery* delivery = clientProxy.delivery_manager.writeSequenceNumber(packet);
					if (delivery)
					{
						delivery->delegate = new ReplicationDeliveryDelegate(&clientProxy.replication);
					}

					clientProxy.replication.write(packet);
...
GameObject * ModuleNetworkingServer::spawnPlayer(uint8 spaceshipType, vec2 initialPosition, float initialAngle)
{
	...
	if (spaceshipType == 0) {
		gameObject->sprite->texture = App->modResources->shotter1;
	}
	else if (spaceshipType == 1) {
		gameObject->sprite->texture = App->modResources->shotter2;
	}
	else if(spaceshipType == 2){
		gameObject->sprite->texture = App->modResources->shotter3;
	}
	else {
		gameObject->sprite->texture = App->modResources->shotter4;
	}
...
void ModuleNetworkingServer::ScoreSearcher(GameObject* go)
{
	for (int i = 0; i < MAX_CLIENTS; ++i)
	{
		if (clientProxies[i].connected)
		{
			...
			{
				clientProxies[i].marcador += 1;
				WLOG("han matado a alguien");
			}
...
void ModuleNetworkingServer::ScoreReset(GameObject* go)
{
	for (int i = 0; i < MAX_CLIENTS; ++i)
	{
		if (clientProxies[i].connected)
